...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const variantTransforms = {
    "variant-1": {
      img: { x: 50, y: 50, opacity: 0 }, // Adjust the x and y values to change position
  ...
  items.forEach((item, index) => {
    item.addEventListener("mouseenter", () => {
      ...
      const elementsToAnimate = ["img", "title", "tags", "description"];
        ...
        if (!isMouseOverItem) {
          ...
          if (activePreview) {
            ...
            const activePreviewImg =
              activePreview.querySelector(".preview-img");
            const defaultClipPaths = getDefaultClipPath(activePreview);
            gsap.to(activePreviewImg, {
              clipPath: defaultClipPaths,
              duration: 1,
              ease: "power3.out",
            });
          }
